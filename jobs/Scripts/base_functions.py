...
def reportToJSON(case, render_time_rpr=0, render_time_cycles=0, render_time_eevee=0):
    ...
    report['render_time'] = render_time_rpr
    report['render_time_cycles'] = render_time_cycles
    report['render_time_eevee'] = render_time_eevee
    ...
    if case['status'] != 'skipped':
        ...
        report['render_color_path'] = path.join('RPR', report['file_name'])
        report['render_color_path_cycles'] = path.join('Cycles', report['file_name'])
        report['render_color_path_eevee'] = path.join('Eevee', report['file_name'])
...
def enable_cycles(case):
    event('Load cycles', True, case)
    set_value(bpy.context.scene.render, 'engine', 'CYCLES')
    event('Load cycles', False, case)

def enable_eevee(case):
    event('Load eevee', True, case)
    set_value(bpy.context.scene.render, 'engine', 'BLENDER_EEVEE')
    event('Load eevee', False, case)
...
def render(case):
    ...
    set_value(scene.render, 'filepath', os.path.join(
        WORK_DIR, 'RPR', case['case']))
    bpy.context.scene.eevee.taa_render_samples = 64
    set_value(scene, 'eevee.taa_render_samples', 64)
    start_time = datetime.datetime.now()
    bpy.ops.render.render(write_still=True)
    render_time_rpr = (datetime.datetime.now() - start_time).total_seconds()

    enable_cycles(case['case'])
    set_value(scene.render, 'filepath', os.path.join(
        WORK_DIR, 'Cycles', case['case']))
    set_value(scene, 'cycles.device', 'GPU')
    set_value(scene, 'cycles.samples', 32)
    start_time = datetime.datetime.now()
    bpy.ops.render.render(write_still=True)
    render_time_cycles = (datetime.datetime.now() - start_time).total_seconds()

    enable_eevee(case['case'])
    set_value(scene.render, 'filepath', os.path.join(
        WORK_DIR, 'Eevee', case['case']))
    set_value(scene, 'rpr.limits.max_samples', 32)
    start_time = datetime.datetime.now()
    bpy.ops.render.render(write_still=True)
    render_time_eevee = (datetime.datetime.now() - start_time).total_seconds()
    event('Postrender', True, case['case'])

    #print(calculate_ssim(os.path.join(WORK_DIR, 'Cycles', case['case']), os.path.join(WORK_DIR, 'RPR', case['case'])))

    reportToJSON(case, render_time_rpr, render_time_cycles, render_time_eevee)
